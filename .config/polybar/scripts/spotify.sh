...
main() {
  ...
  echo "${*:-%artist% - %title%}" | sed "s/%artist%/$artist/g;s/%title%/$title/g;s/%album%/$album/g"i | sed "s/\&/\&/g" | sed "s#\/#\/#g" > ~/.config/polybar/scripts/spotify_current
  cat ~/.config/polybar/scripts/spotify_current
}
